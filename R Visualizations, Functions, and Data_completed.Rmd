...
cars_data <- select(mtcars,
                      starts_with("m"),
                      ends_with('g'),
                      matches('mp|c'),
                      )
cars_data_mut <- mutate(mtcars,
                          power = cyl/disp,
                          )
cars_data_mut 
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    get_count <- function(input_str){
     ...
     for (i in unlist(strsplit(input_str, ""))) {
    ...
    descending_order <- function(num){
      ...
      v <- as.numeric(strsplit(as.character(num),"")[[1]])

      sort_arr <- sort(v, decreasing=TRUE)

      sort_num = paste(sort_arr,collapse='')

      return (as.numeric(sort_num))
...
3.  **Mario Challenge:** In the classic Super Mario games, Mario ends
    ...
    for (i in 1:4){
      ...
      print(steps,quote=FALSE)
      steps = c()
    }
    # Yo may feel more comfortable figuring this out in Python first, then translating into R later.
...
```{r}
air_data_mut <- mutate(airquality,
                          OT = Ozone/Temp,
                         ODP=Ozone/Solar.R )
air_data_mut 
...
ggplot(data=airquality) +
  geom_point(mapping = aes(Ozone, Temp))
```

```{r}
ggplot(data = airquality) +
  geom_point(mapping = aes(Ozone,Temp, colour =rgb(0.7,1.0,.3)))+
  facet_wrap(~Day, nrow = 3)
```

------------------------------------------------------------------------

Observation: the ozone has relationship with respect to temperature

Question: is there a relationship between ozone with respect to
temperature?

Hypothesize: null hypothesis-there is no relationship between the ozone
with respect to temperature alternate hypothesis-there is a relationship
between the ozone with respect to temperature

Test: A comparison test would be performed and then reject or fail to
reject one of the hypothesis

Conclusion: Analyse the results and draw conclusions depending on
